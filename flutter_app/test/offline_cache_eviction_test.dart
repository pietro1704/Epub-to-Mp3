...
void main() {
  ...
  group('OfflineCacheEviction', () {
    test('under budget and within TTL — no eviction', () async {
      ...
      expect(Directory('${tempDir.path}/downloads/book1').existsSync(), isTrue);
      expect(Directory('${tempDir.path}/downloads/book2').existsSync(), isTrue);
    });

    test('automatic maintenance preserves expired completed audio', () async {
      final now = DateTime.now().toUtc();
    ...
    test(
      'automatic maintenance preserves audio above the old budget',
      () async {
        final now = DateTime.now().toUtc();
        ...
        expect(evicted, isEmpty);
        expect(
          Directory('${tempDir.path}/downloads/book_a').existsSync(),
          isTrue,
        );
        expect(
          Directory('${tempDir.path}/downloads/book_c').existsSync(),
          isTrue,
        );
      },
    );
    ...
    test(
      'automatic maintenance preserves active and inactive completed audio',
      () async {
        final now = DateTime.now().toUtc();
        ...
        expect(evicted, isEmpty);
        expect(
          Directory('${tempDir.path}/downloads/playing').existsSync(),
          isTrue,
        );
      },
    );
    ...
    test(
      'automatic maintenance preserves all completed audio above budget',
      () async {
        final now = DateTime.now().toUtc();
        ...
        for (var i = 0; i < ids.length; i++) {
          await plantAudiobook(
            ...
            jobId: ids[i],
            totalBytes: 1000,
            lastAccessedAt: now.subtract(Duration(hours: ids.length - i)),
          );
        }
        ...
        for (final id in ids) {
          expect(
            Directory('${tempDir.path}/downloads/$id').existsSync(),
            isTrue,
          );
        }
      },
    ...
    test('touchLastAccess writes sidecar file', () async {
      ...
      final parsed = DateTime.tryParse(await sidecar.readAsString())?.toUtc();
    ...
    test('constants are reasonable', () {
      ...
      expect(kDefaultOfflineCacheTTLSeconds, equals(24 * 60 * 60)); // 24 h
    });
  });
}
