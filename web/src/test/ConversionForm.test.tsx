...
describe('ConversionForm', () => {
  it('exibe mensagem de erro quando nenhum arquivo é selecionado', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByRole('alert')).toHaveTextContent('Selecione um arquivo EPUB ou PDF antes de enviar.');
    });
  ...
  it('envia os valores selecionados usando upload automático', async () => {
    ...
    await user.selectOptions(screen.getByLabelText(/nome da voz/i), 'tts_models/pt/cv/vits');
  ...
  it('mostra status enquanto detecta capa automaticamente', async () => {
    ...
    const handleSubmit = vi.fn();
    const handleUpload = vi.fn().mockImplementation(() => new Promise<UploadResponse>((resolve) => {
    ...
    expect(screen.getByRole('button', { name: /converter agora/i })).toBeDisabled();

    resolveUpload?.({ uploadId: 'auto-id', fileName: 'amostra.epub' });
    await waitFor(() => expect(screen.getByText(/metadados detectados/i)).toBeInTheDocument());
    expect(screen.getByRole('button', { name: /converter agora/i })).toBeEnabled();
  });
});
