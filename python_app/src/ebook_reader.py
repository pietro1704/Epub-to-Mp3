...
class TextProcessor:
    ...
    def _collect_footnotes_bs4(
        ...
        # Casefolded set of the document's heading texts. A "footnote" whose
        # text is identical to a heading is spurious: it happens when a nav /
        # jump anchor points at a section heading (e.g. a "Maps" plate whose
        ...
        for anchor in list(all_anchors):
            ...
            if not looks_like_noteref(anchor, note_text, note_node):
                continue
            # Reject a note whose text is just a section heading of this
            # document (a nav/jump anchor mistaken for a noteref). Otherwise
            # an image-only plate like "Maps" is spoken as
            # "Maps. footnote 1... Maps. end of footnote...".
            if _heading_texts:
                _note_key = TextProcessor.normalise_whitespace(note_text).rstrip(".!?;:").casefold()
                if _note_key in _heading_texts:
                    continue
...
class EbookReader:
    ...
    def load(
        ...
        if suffix not in {".epub", ".pdf", ".fb2", ".cbz", ".cbr", ".docx"} | _mobi_suffixes:
        ...
        if suffix == ".epub":
            ...
            self.book = CbrParser(str(path)).parse()
        elif suffix == ".docx":
            from .docx_parser import DocxParser

            self.book = DocxParser(str(path)).parse()
        elif suffix in _mobi_suffixes:
            from .mobi_drm import raise_if_drm_protected
            from .mobi_parser import MobiParser

            raise_if_drm_protected(path)
            self.book = MobiParser(str(path)).parse()
        else:
    ...
    def extract_cover_image(self) -> Optional[CoverImage]:
        """Return the raw cover image bundled in the source, if available."""
        if not self.file_path:
            return None
        ...
        # CBZ cover extraction: the first page image, in reading order.
        elif suffix in {".cbz", ".cbr"}:
            if not self.book or not self.book.chapters:
                return None
            first_page = self.book.chapters[0]
            try:
                if suffix == ".cbz":
                    with zipfile.ZipFile(self.file_path, "r") as archive:
                        data = archive.read(first_page.source_path)
                    media_type = mimetypes.guess_type(first_page.source_path)[0] or "image/jpeg"
                    extension = Path(first_page.source_path).suffix or ".jpg"
                else:
                    from .cbr_parser import read_page

                    data, media_type, extension = read_page(self.file_path, first_page.source_path)
            except Exception:
                return None
            return CoverImage(data=data, media_type=media_type, extension=extension)
    ...
    def extract_chapter_resources(self, chapter: Chapter) -> list[dict[str, str]]:
        """Return bounded image resources referenced by a chapter, or the
        single page image for CBZ/CBR chapters."""
        if not self.file_path:
            return []
        suffix = self.file_path.suffix.lower()
        ...
        if suffix in {".cbz", ".cbr"}:
            ...
            try:
                if suffix == ".cbz":
                    with zipfile.ZipFile(self.file_path, "r") as archive:
                        data = archive.read(chapter.source_path)
                    media_type = mimetypes.guess_type(chapter.source_path)[0] or "image/jpeg"
                else:
                    from .cbr_parser import read_page

                    data, media_type, _ = read_page(self.file_path, chapter.source_path)
            except Exception:
                return []
            return [
                {
...
def parse_epub_to_dict(file_path: str, book_id: str = "") -> dict:
    ...
    ``contentKind`` is ``"images"`` for CBZ/CBR (one page image per chapter,
    via ``resources``, ``text`` intentionally empty and not dropped) and
    ``"text"`` for every other format — the client uses it to decide
    whether to render a text chapter or an image page, and to disable
    TTS conversion for image-only books.
    """
    reader = EbookReader(file_path)
    if not reader.book:
        raise RuntimeError("Failed to read book")
    book = reader.book
    # CBZ chapters are legitimately textless (one page image each) — the
    # empty-text drop below only applies to text-based formats, where an
    # empty chapter really is content-free (cover pages, blank separators).
    is_image_only = reader.file_path is not None and reader.file_path.suffix.lower() in {
        ".cbz",
        ".cbr",
    }
    content_kind = "images" if is_image_only else "text"
    chapters: list[dict] = []
