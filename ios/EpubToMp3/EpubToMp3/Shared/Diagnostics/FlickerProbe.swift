...
final class FlickerProbe: ObservableObject {
    ...
    /// Serial queue for the blocking file-write part of `log`.
    ...
    /// stdout synchronously on the main thread at that rate added enough
    ...
    /// Redirect the diagnostic file for host tests so they never contend with
    /// a real app instance or a stale Documents file. Device and production
    /// callers keep using Documents/flicker-debug.log.
    func useTemporaryDebugFileForTests() {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("flicker-debug-\(ProcessInfo.processInfo.processIdentifier).log")
    ...
    func log(_ message: String) {
        ...
        // File writes are the only blocking operation — do them off-main and
        // serialize them so line order is preserved. Do not put stdout on
        // this queue: XCTest and host log capture can block a pipe, which
        // would prevent the flush helper from ever returning.
        ioQueue.async {
