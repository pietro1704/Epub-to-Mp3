...
struct ReaderView: View {
    ...
    /// host can mirror `currentChapterIndex` (TOC highlight, position persistence).
    /// Zero-based EPUB chapter index.
    var onScrolledToChapter: ((Int) -> Void)? = nil
    /// Stable book/job identity used to isolate rendered chapter caches when
    /// two books contain the same chapter index.
    ...
    /// At most one neighbour-render pass may be pending. Rapid chapter
    /// changes otherwise queue duplicate WebKit/TextKit work on the main
    /// actor after the user has already left the chapter.
    ...
    init(
        ...
        self.renderNamespace = renderNamespace
