...
@MainActor
enum ReaderAttributedRenderCache {
    ...
    /// Build a cache key that cannot collide when two books contain a
    /// chapter with the same index and reader settings. `chapter.id` is only
    /// the EPUB chapter index, so the book/job namespace is part of the
    /// identity even though the rendered payload is chapter-local.
    static func key(
        namespace: String,
        chapterID: String,
        settingsKey: String
    ) -> String {
        [namespace, chapterID, settingsKey].joined(separator: "\u{1F}")
    }
...
/// Compatibility name retained for existing call sites and tests.
typealias BookChapterRenderCache = ReaderAttributedRenderCache

struct BookChapterCell: View {
    let chapter: EbookFulltext.Chapter
    /// Stable book/job identity used to isolate renders across books.
    ...
    static func renderKey(
        ...
        return ReaderAttributedRenderCache.key(
            namespace: namespace,
    ...
    var body: some View {
        ...
        .task(id: renderKey) {
            ...
            #if os(iOS)
            let chapterMarker = "\u{1F}\(chapter.id)\u{1F}"
            if lastRenderKey != nil, lastRenderKey?.contains(chapterMarker) == true {
