...
enum EpubFallbackParser {
    ...
    fileprivate static func extractImageResources(
        ...
        let archiveEntries = ZipReader.listEntries(in: archiveURL) ?? []
        ...
        for match in pattern.matches(in: html, range: NSRange(html.startIndex..., in: html)) {
            ...
            guard let archivePath = resolveArchiveMember(candidate, entries: archiveEntries) else {
                continue
            }
    ...
    /// Resolve a filesystem-normalized resource candidate back to a ZIP
    /// member. Relative EPUB paths normally match directly; malformed EPUBs
    /// can make Foundation prepend the host filesystem path, so use the
    /// longest archive-entry suffix only after exact matching fails.
    static func resolveArchiveMember(_ candidate: String, entries: [String]) -> String? {
        let normalized = candidate
            .split(separator: "/", omittingEmptySubsequences: true)
            .joined(separator: "/")
        guard !normalized.isEmpty else { return nil }
        guard !entries.isEmpty else { return normalized }
        if entries.contains(normalized) { return normalized }
