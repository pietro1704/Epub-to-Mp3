...
class EbookReader:
    ...
    def extract_chapter_resources(self, chapter: Chapter) -> list[dict[str, str]]:
        """Return bounded image resources referenced by an EPUB chapter, or
        (for CBZ) the single page image itself."""
        if not self.file_path:
            return []
        suffix = self.file_path.suffix.lower()
...
def parse_epub_to_dict(file_path: str, book_id: str = "") -> dict:
    ...
    ``contentKind`` is ``"images"`` for CBZ (one page image per chapter, via
    ``resources``, ``text`` intentionally empty and not dropped) and
    ...
    is_image_only = reader.file_path is not None and reader.file_path.suffix.lower() == ".cbz"
