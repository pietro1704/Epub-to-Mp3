        ...
        for idx, chapter in enumerate(chapters):
            ...
            try:
                ...
                output_path = FileManager.get_output_path(
                    chapter.name, output_dir, chapter_num
                )
                ...
                if result and output_path.exists():
                    ...
                    print(f"  ✅ Sucesso ({elapsed:.1f}s, {file_size:,} bytes, {chars_per_sec:.0f} chars/s)")
                else:
    ...
    async def _convert_parallel_simple(
        ...
        for idx, chapter in enumerate(chapters):
            task = self._process_chapter_with_semaphore(
                semaphore, chapter, idx + 1, len(chapters),
                tts_engine, output_dir, config
            )
            tasks.append(task)
        ...
        async with semaphore:
            ...
            try:
                ...
                if output_path.exists() and not config.force_reprocess:
                    elapsed = time.time() - start_time
                    file_size = output_path.stat().st_size
                    print(f"✅ [{chapter_num}/{total_chapters}] Já existe ({elapsed:.1f}s, {file_size:,} bytes)")
                ...
                if result and output_path.exists():
                    ...
                    file_size = output_path.stat().st_size
                    chars_per_sec = len(chapter.text) / synthesis_time if synthesis_time > 0 else 0
                    print(f"✅ [{chapter_num}/{total_chapters}] Sucesso ({elapsed:.1f}s, {file_size:,} bytes, {chars_per_sec:.0f} chars/s)")
                    return {"success": True, "path": str(output_path)}
                else:
                    error_msg = f"Falha na síntese após {synthesis_time:.1f}s"
                    if hasattr(tts_engine, 'last_error') and tts_engine.last_error:
                        error_msg += f": {tts_engine.last_error}"
                    elapsed = time.time() - start_time
                    print(f"❌ [{chapter_num}/{total_chapters}] Falha ({elapsed:.1f}s): {error_msg}")
